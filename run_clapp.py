...
from configs.task_configs import CLAPPConfig
...
from tasks.clapp import CLAPPLoss
...
def main():
    """Main function for single or distributed CLAPP training."""

    config = CLAPPConfig.parse_arguments()
...
def main_worker(local_rank: int, config: object):
    ...
    model = CLAPP(
        ...
        head=head,
        queue=MemoryQueue(
            size=(config.projector_dim, config.num_negatives),
            device=local_rank,
        ),
        loss_function=CLAPPLoss(
